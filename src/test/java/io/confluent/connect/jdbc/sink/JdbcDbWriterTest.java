...
public class JdbcDbWriterTest {
  ...
  @Test
  public void shouldWorkWithCommitRecordInDifferentCall() throws SQLException {
    String topic = "books";
    TableId tableId = new TableId(null, null, topic);
    ...
    // The above call did not have any commit record yet, so there should not be any records in
    // target database yet.
    assertEquals(
      1,
    ...
    // This call would cause the connection to commit the records to the target database.
    writer.writeConsistently(Collections.singletonList(
      new SinkRecord(topic, 0, txnSchema, endStruct, txnSchema, endStruct, 2)));
